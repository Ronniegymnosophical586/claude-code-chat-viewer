...
HTML-вьюер для транскриптов сессий [Claude Code](https://claude.com/claude-code) в формате JSONL. Открывается в браузере, без сервера, без сборки, одна локальная зависимость. Работает офлайн из коробки.
