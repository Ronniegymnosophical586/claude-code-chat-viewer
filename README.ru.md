...
HTML-вьюер для транскриптов сессий [Claude Code](https://claude.com/claude-code) в формате JSONL. Открывается в браузере, без сервера, без сборки, одна локально вендоренная зависимость. Работает офлайн из коробки.
...
- **XSS-безопасный рендер** — каждый текстовый блок экранируется _до_ передачи в markdown-парсер. Markdown-картинки нейтрализуются (показываются как инертный текст, не загружаются). Ссылки ограничены `http(s)` с `rel="noopener noreferrer nofollow"`.
